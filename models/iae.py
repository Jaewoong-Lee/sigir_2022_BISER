...
class iAE(AbstractRecommender):
    def __init__(self, sess, data, train, val, test, num_users: np.array, num_items: np.array, \
        ...
        self.val = val
        self.test = test
        self.train_dict = csr_to_user_dict(tocsr(train, num_users, num_items))
        self.sess = sess
    ...
    def create_placeholders(self):
        with tf.name_scope("input_data"):
            self.input_R = tf.placeholder(tf.float32, [None, self.num_users])
            self.tmp1 = tf.placeholder(tf.float32, [None, self.num_users])
            self.tmp2 = tf.placeholder(tf.float32, [None, self.num_users])
            self.tmp3 = tf.placeholder(tf.float32, [None, self.num_users])
    ...
    def build_graph(self):
        with tf.name_scope("embedding_layer"):  # The embedding initialization is unknown now
            ...
            self.weights = {'encoder': tf.Variable(initializer([self.num_users, self.hidden_dim])),
                            'decoder': tf.Variable(initializer([self.hidden_dim, self.num_users]))}
            self.biases = {'encoder': tf.Variable(initializer([self.hidden_dim])),
                           'decoder': tf.Variable(initializer([self.num_users]))}

        with tf.name_scope("prediction"):
            input_R = self.input_R # no mask
    ...
    def train_model(self, pscore, unbiased_eval):
        ...
        all_tr = np.arange(self.num_items)
        for epoch in range(self.num_epochs):
            ...
            for b in range(batch_num):
                ...
                for idx, item_id in enumerate(batch_set_idx):
                    users_by_item_id = self.train_dict[item_id]
                    batch_matrix[idx, users_by_item_id] = 1
